    ...
    <div className={`configuration-page ${embedded ? "embedded" : ""}`}>
      ...
      <div className="configuration-layout">
        ...
        <main className="configuration-content">
          ...
          {tab === "platforms" && <><div className="config-intro"><span className="eyebrow">Control de fichas</span><h2>Plataformas</h2><p>Administrá las plataformas, los nombres, colores y subplataformas de cada una.</p></div><div style={{ display: "flex", gap: "12px", marginBottom: "24px" }}><button className="config-add" type="button" onClick={async () => { if (await confirmDelete("¿Vaciar TODAS las subplataformas de la configuración? Esto no se puede deshacer.")) setDraft((current) => ({ ...current, platformSubPlatforms: {} })); }} style={{ background: "rgba(255, 90, 90, 0.2)", color: "#ff5a5a", borderColor: "rgba(255, 90, 90, 0.3)" }} title="Vaciar todas las subplataformas"><Trash2 size={15} /> Limpiar todas subplataformas</button></div><PlatformConfigList platforms={draft.platforms} platformEntities={draft.platformEntities} onEntitiesChange={(platformEntities) => setDraft((current) => ({ ...current, platformEntities }))} platformColors={draft.platformColors || {}} onPlatformsChange={(platforms) => setDraft((current) => ({ ...current, platforms }))} onColorChange={(platform, color, previous) => setDraft((current) => { const platformColors = { ...(current.platformColors || {}), [platform]: color }; if (previous) { delete platformColors[previous]; return { ...current, platforms: current.platforms.map((item) => item === previous ? platform : item), platformColors }; } return { ...current, platformColors }; })} platformSubPlatforms={draft.platformSubPlatforms || {}} onSubPlatformsChange={(platformSubPlatforms) => setDraft((current) => ({ ...current, platformSubPlatforms }))} /></>}

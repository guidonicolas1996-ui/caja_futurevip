...
function TransferSection({ caja, config, boxes, activeBoxId, transfers, savingsMovements = [], onCreate, onUpdateTransfer, onDeleteTransfer, onCreateSavings, onUpdateSavings, onDeleteSavings }) {
  ...
  return (
    <section className="transfer-panel">
      ...
      {editorOpen && <div className="modal-backdrop" onClick={() => setEditorOpen(false)}><div className="modal transfer-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setEditorOpen(false)}><X size={18} /></button><div className="modal-icon"><ArrowLeftRight size={20} /></div><h2>Traspasos del turno</h2><p>Editá el origen, destino, monto o nota de cada movimiento.</p><div className="transfer-edit-list">{transfers.length === 0 && <div className="empty-state">Todavía no hay traspasos.</div>}{transfers.map((transfer) => <div className="transfer-edit-row" key={transfer.id}><time className="movement-time">{formatMovementTime(transfer.createdAt)}</time><div><b>{boxes.find((box) => box.id === transfer.fromBoxId)?.title}</b><span>hacia {boxes.find((box) => box.id === transfer.toBoxId)?.title}</span></div><strong>{money(transfer.amount)}</strong><button className="icon-button" title="Editar traspaso" onClick={() => setEditingMovement({ ...transfer })}><Settings2 size={14} /></button><button className="delete-button" title="Eliminar traspaso" onClick={() => setDeleteMovementId(transfer.id)}><Trash2 size={14} /></button></div>)}</div><div className="modal-actions"><button className="close-button" onClick={() => setEditorOpen(false)}>Listo <Check size={16} /></button></div></div></div>}
      ...
      {deleteMovementId && <ConfirmDialog dialog={{ message: "¿Seguro que querés eliminar este traspaso de ambas cajas?", onConfirm: async () => { await onDeleteTransfer(deleteMovementId); setDeleteMovementId(null); } }} onClose={() => setDeleteMovementId(null)} />}
    </section>
  );
}

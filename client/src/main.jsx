...
function BonusesPage({ config, activeBoxId, api, onNotify }) {
  ...
  const conditionsForType = (typeId, existing = []) => { const count = types.find((type) => type.id === typeId)?.percentageCount || 0; return Array.from({ length: count }, (_, index) => existing[index] ? { ...existing[index] } : conditions[index] ? { conditionId: conditions[index].id, percentage: "" } : { conditionId: "", percentage: "" }); };
      ...
      <main>
        <div className="page-title">
          ...
          <div className="history-actions">
             {(statisticsOpen || logisticsOpen || usersOpen || bonusesOpen || configurationOpen) && <button className="history-trigger" title="Volver a Caja" aria-label="Volver a Caja" onClick={() => { setStatisticsOpen(false); setLogisticsOpen(false); setUsersOpen(false); setBonusesOpen(false); setConfigurationOpen(false); }}><span aria-hidden="true">🏠</span></button>}
             {!statisticsOpen && <button className="history-trigger statistics-trigger" title="Estadísticas" aria-label="Estadísticas" onClick={() => { setStatisticsOpen(true); setConfigurationOpen(false); setLogisticsOpen(false); setUsersOpen(false); setBonusesOpen(false); setBonusViewRequest(0); setBonusEditorRequest(0); }}><span aria-hidden="true">📊</span></button>}
             {!logisticsOpen && <button className="history-trigger logistics-trigger" title="Logística" aria-label="Logística" onClick={() => { setLogisticsOpen(true); setConfigurationOpen(false); setStatisticsOpen(false); setUsersOpen(false); setBonusesOpen(false); setBonusViewRequest(0); setBonusEditorRequest(0); }}><span aria-hidden="true">📦</span></button>}
             {!usersOpen && <button className="history-trigger users-trigger" title="Usuarios" aria-label="Usuarios" onClick={() => { setUsersOpen(true); setBonusesOpen(false); setConfigurationOpen(false); setStatisticsOpen(false); setLogisticsOpen(false); setBonusViewRequest(0); setBonusEditorRequest(0); }}><span aria-hidden="true">👥</span></button>}
             {!bonusesOpen && <button className="history-trigger bonuses-trigger" title="Bonos" aria-label="Bonos" onClick={() => { setBonusesOpen(true); setUsersOpen(false); setConfigurationOpen(false); setStatisticsOpen(false); setLogisticsOpen(false); setBonusViewRequest(0); setBonusEditorRequest(0); }}><span aria-hidden="true">🎁</span></button>}
             <button className="history-trigger" title="Cajas recientes" aria-label="Cajas recientes" onClick={() => setHistoryOpen(true)}><span aria-hidden="true">🕒</span></button>
             {!configurationOpen && <button className="history-trigger" disabled={readOnly} title="Configurar" aria-label="Configurar" onClick={() => { setConfigurationOpen(true); setStatisticsOpen(false); setLogisticsOpen(false); setUsersOpen(false); setBonusesOpen(false); }}><span aria-hidden="true">⚙️</span></button>}

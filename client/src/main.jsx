...
function TransferSection({ caja, config, boxes, activeBoxId, transfers, savingsMovements = [], onCreate, onUpdateTransfer, onDeleteTransfer, onCreateSavings, onUpdateSavings, onDeleteSavings }) {
  ...
  const saveTransfer = async () => {
    try { await onUpdateTransfer(editingMovement); setEditingMovement(null); }
    catch (requestError) { setError(requestError.message); }
  };
  return (
    <section className="transfer-panel">
      ...
      <div className="transfer-history"><div className="recent-movements"><span>{savingsMode ? "Últimos movimientos de ahorro" : "Últimos movimientos"}</span>{records.slice().reverse().map((record) => savingsMode ? <div className="recent-movement transfer-row" key={record.id}><span>{record.holder} · {record.wallet}{record.note ? ` · ${record.note}` : ""}</span><b className="transfer-out">-{money(record.amount)}</b></div> : (() => { const outgoing = record.fromBoxId === activeBoxId; const otherBox = boxes.find((box) => box.id === (outgoing ? record.toBoxId : record.fromBoxId)); return <div className="recent-movement transfer-row" key={record.id}><span>{outgoing ? "Salida a" : "Entrada de"} {otherBox?.title || "otra caja"}{record.note ? ` · ${record.note}` : ""}</span><b className={outgoing ? "transfer-out" : "transfer-in"}>{outgoing ? "-" : "+"}{money(record.amount)}</b></div>; })())}{records.length === 0 && <small>{savingsMode ? "Sin movimientos de ahorro todavía" : "Sin movimientos todavía"}</small>}</div></div>
      {editorOpen && <div className="modal-backdrop" onClick={() => setEditorOpen(false)}><div className="modal transfer-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setEditorOpen(false)}><X size={18} /></button><div className="modal-icon"><ArrowLeftRight size={20} /></div><h2>{savingsMode ? "Movimientos de ahorro" : "Movimientos entre cajas"}</h2><p>Editá el origen, destino, monto o nota de cada movimiento.</p><div className="transfer-edit-list">{records.length === 0 && <div className="empty-state">Todavía no hay movimientos.</div>}{records.map((record) => <div className="transfer-edit-row" key={record.id}><time className="movement-time">{formatMovementTime(record.createdAt)}</time><div>{savingsMode ? <><b>{record.holder}</b><span>{record.wallet}{record.note ? ` · ${record.note}` : ""}</span></> : <><b>{boxes.find((box) => box.id === record.fromBoxId)?.title}</b><span>hacia {boxes.find((box) => box.id === record.toBoxId)?.title}</span></>}</div><strong>{money(record.amount)}</strong>{!savingsMode && <button className="icon-button" title="Editar movimiento" onClick={() => setEditingMovement({ ...record })}><Settings2 size={14} /></button>}<button className="delete-button" title="Eliminar movimiento" onClick={() => setDeleteMovementId(record.id)}><Trash2 size={14} /></button></div>)}</div><div className="modal-actions"><button className="close-button" onClick={() => setEditorOpen(false)}>Listo <Check size={16} /></button></div></div></div>}
      {editingMovement && <div className="modal-backdrop" onClick={() => setEditingMovement(null)}><div className="modal transfer-edit-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => setEditingMovement(null)}><X size={18} /></button><div className="modal-icon"><Settings2 size={20} /></div><h2>Editar traspaso</h2><div className="transfer-edit-fields"><TransferBoxPicker label="Desde" boxes={boxes} value={editingMovement.fromBoxId} onChange={(value) => setEditingMovement({ ...editingMovement, fromBoxId: value })} /><TransferBoxPicker label="Hasta" boxes={boxes} value={editingMovement.toBoxId} excludeId={editingMovement.fromBoxId} onChange={(value) => setEditingMovement({ ...editingMovement, toBoxId: value })} /><label><span>Monto</span><AmountInput value={editingMovement.amount} onChange={(value) => setEditingMovement({ ...editingMovement, amount: value })} /></label><label><span>Nota</span><input value={editingMovement.note || ""} onChange={(event) => setEditingMovement({ ...editingMovement, note: event.target.value })} /></label></div><div className="modal-actions"><button className="ghost-button" onClick={() => setEditingMovement(null)}>Cancelar</button><button className="close-button" onClick={saveTransfer}>Guardar <Check size={16} /></button></div></div></div>}
      {deleteMovementId && <ConfirmDialog dialog={{ message: "¿Seguro que querés eliminar este movimiento?", onConfirm: async () => { await (savingsMode ? onDeleteSavings(deleteMovementId) : onDeleteTransfer(deleteMovementId)); setDeleteMovementId(null); } }} onClose={() => setDeleteMovementId(null)} />}
    </section>
  );
}
...
function BonusMonthlyGoalProgress({ config, caja, history, boxColor }) {
  ...
  const colors = boxColorStyle(boxColor);
  const renderBar = (label, value, target, percent, targetMarkerPercentage = null, targetMarkerLabel = "Objetivo del día") => {
  ...
  return <div className="bonus-goal-panel" aria-label="Progreso del objetivo de bonos" style={{ "--bonus-soft": colors["--box-soft"], "--bonus-line": colors["--box-line"], "--bonus-glow": colors["--box-glow"], "--bonus-accent": colors["--box-accent"] }}>
    ...
    <div className="bonus-goal-lower-row">
      {renderBar("Obj. Bonos Día", dayBonusNet, dailyTarget, dailyTarget > 0 ? (dayBonusNet / dailyTarget) * 100 : 0, cumulativeShiftPercentage(goal.percentages, currentShift), "Objetivo del turno")}

...
function MiniUsersPanel({ config, boxes, activeBoxId, onConfigChange, onNotify }) {
  ...
  const saveNewUser = () => {
    ...
    onConfigChange({ ...config, users: [...users, { id: `user-${crypto.randomUUID()}`, names: [name], phones: [phone], titulars: newUser.titular.trim() ? [newUser.titular.trim()] : [], boxes: [newUser.boxId || activeBoxId], subPlatforms: [], clarifications: [], linkedUsers: [], createdAt: new Date().toISOString() }] });
  ...
  return <section className="panel mini-users-panel">
    ...
    <div className="mini-users-search"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar usuario" aria-label="Buscar usuario" /><button className="icon-button" type="button" title="Dar de alta usuario" aria-label="Dar de alta usuario" onClick={() => { setNewUser({ name: "", phone: "", titular: "", boxId: activeBoxId }); setNewUserOpen(true); }}><UserPlus size={16} /></button></div>
    ...
    {selectedUser && <div className="modal-backdrop" onClick={() => setSelectedUser(null)}><div className="modal mini-user-modal user-detail-popup" onClick={(event) => event.stopPropagation()}><button className="modal-close" type="button" title="Cerrar" onClick={() => setSelectedUser(null)}><X size={18} /></button><div className="modal-icon"><Users size={21} /></div><h2>{displayName(selectedUser)}</h2><div className="user-fields-grid mini-user-fields"><label className="field-block"><span>Nombre de usuario</span><input readOnly value={displayName(selectedUser)} /></label><label className="field-block"><span>Número de teléfono</span><input readOnly value={selectedUser.phones?.join(" / ") || selectedUser.phone || "Sin teléfono"} /></label><label className="field-block"><span>Titular</span><input readOnly value={selectedUser.titulars?.join(" / ") || selectedUser.titular || "Sin titular"} /></label><label className="field-block"><span>Panel</span><input readOnly value={panelValueFor(selectedUser) || "Sin panel"} /></label><label className="field-block"><span>Fecha creación</span><input readOnly value={selectedUser.createdAt ? new Date(selectedUser.createdAt).toLocaleDateString("es-AR") : "Sin fecha"} /></label></div><div className="user-checks-grid mini-user-sections"><div className="check-group"><span>Cajas</span><div className="checkbox-list">{(selectedUser.boxes || []).map((boxId) => <span className="user-switch" key={boxId}><i /><span>{boxById[String(boxId)]?.title || "Sin caja"}</span></span>)}</div></div><div className="check-group"><span>Plataformas</span><div className="checkbox-list">{(selectedUser.subPlatforms || []).map((key) => <span className="user-switch" key={key}><i /><span>{key.split("::").at(-1)}</span></span>)}{!selectedUser.subPlatforms?.length && <small>Sin plataformas</small>}</div></div></div><div className="user-clarifications mini-user-block"><span>Aclaraciones</span><div className="checkbox-list compact">{(selectedUser.clarifications || []).map((id) => <span className="clarification-pill user-switch" key={id}><i /><span>{config.userClarifications?.find((item) => item.id === id)?.text || "Aclaración"}</span></span>)}{!selectedUser.clarifications?.length && <small>Sin aclaraciones</small>}</div></div><div className="user-linked-section mini-user-block"><span>Usuarios vinculados</span><div className="linked-tags">{(selectedUser.linkedUsers || []).map((id) => users.find((item) => item.id === id)).filter(Boolean).map((user) => <span className="linked-tag" key={user.id}>{displayName(user)}</span>)}{!selectedUser.linkedUsers?.length && <small>Sin vínculos</small>}</div></div><div className="modal-actions"><button className="close-button" type="button" onClick={() => setSelectedUser(null)}>Listo <Check size={16} /></button></div></div></div>}
    {newUserOpen && <div className="modal-backdrop" onClick={() => setNewUserOpen(false)}><div className="modal users-create-modal mini-user-create-modal" onClick={(event) => event.stopPropagation()}><button className="modal-close" type="button" title="Cerrar" onClick={() => setNewUserOpen(false)}><X size={18} /></button><div className="modal-icon"><UserPlus size={21} /></div><h2>Nuevo usuario</h2><p>Completá los datos obligatorios para darlo de alta.</p><div className="mini-user-form"><label><span>Nombre de usuario *</span><input value={newUser.name} onChange={(event) => setNewUser((current) => ({ ...current, name: event.target.value }))} /></label><label><span>Número de teléfono *</span><input value={newUser.phone} onChange={(event) => setNewUser((current) => ({ ...current, phone: event.target.value }))} /></label><label><span>Titular</span><input value={newUser.titular} onChange={(event) => setNewUser((current) => ({ ...current, titular: event.target.value }))} /></label><label><span>Caja</span><select value={newUser.boxId || ""} onChange={(event) => setNewUser((current) => ({ ...current, boxId: event.target.value }))}>{boxes.map((box) => <option value={box.id} key={box.id}>{box.title}</option>)}</select></label></div><div className="modal-actions"><button className="ghost-button" type="button" onClick={() => setNewUserOpen(false)}>Cancelar</button><button className="close-button" type="button" onClick={saveNewUser}>Guardar <Check size={16} /></button></div></div></div>}
  </section>;
}

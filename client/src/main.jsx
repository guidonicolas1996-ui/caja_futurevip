    ...
    <div className={`configuration-page ${embedded ? "embedded" : ""}`}>
      ...
      <div className="configuration-layout">
        ...
        <main className="configuration-content">
          ...
          {tab === "platforms" && <><div className="config-intro"><span className="eyebrow">Control de fichas</span><h2>Plataformas</h2><p>Administrá las plataformas, los nombres, colores y subplataformas de cada una.</p></div><PlatformConfigList platforms={draft.platforms} platformEntities={draft.platformEntities} onEntitiesChange={(platformEntities) => setDraft((current) => ({ ...current, platformEntities }))} platformColors={draft.platformColors || {}} onPlatformsChange={(platforms) => setDraft((current) => ({ ...current, platforms }))} onColorChange={(platform, color, previous) => setDraft((current) => { const platformColors = { ...(current.platformColors || {}), [platform]: color }; if (previous) { delete platformColors[previous]; return { ...current, platforms: current.platforms.map((item) => item === previous ? platform : item), platformColors }; } return { ...current, platformColors }; })} platformSubPlatforms={draft.platformSubPlatforms || {}} onSubPlatformsChange={(platformSubPlatforms) => setDraft((current) => ({ ...current, platformSubPlatforms }))} /></>}

...
function BonusesPage({ config, activeBoxId, api, onNotify }) {
  ...
  const percentageFor = (bonus) => number(bonus.conditions?.[0]?.percentage);
  ...
  const groups = sorted.reduce((result, bonus) => {
    const groupValue = groupBy === "type" ? typeNameFor(bonus) : groupBy === "percentage" ? `${percentageFor(bonus)}%` : "Todos los bonos";
  ...
  else if (groupBy === "type") groups.sort((left, right) => left.label.localeCompare(right.label, "es", { sensitivity: "base" }));
  ...
  return (
    <main className="bonuses-page">
      <section className="panel bonuses-panel">
        <div className="bonuses-head"><div><h2><Gift size={18} /> Bonos</h2><span>{bonuses.length} registros</span></div><div className="bonuses-toolbar"><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar bono, porcentaje o condición" /><select value={typeFilter} onChange={(event) => setTypeFilter(event.target.value)}><option value="">Todos los tipos</option>{types.map((type) => <option value={type.id} key={type.id}>{type.name}</option>)}</select><label><span>Ordenar</span><select value={sortBy} onChange={(event) => setSortBy(event.target.value)}><option value="percentage">Porcentaje</option><option value="name">Nombre</option><option value="type">Tipo de bono</option><option value="date">Fecha de alta</option></select></label><button className="sort-direction" type="button" title={sortDirection === "asc" ? "Orden ascendente" : "Orden descendente"} aria-label={sortDirection === "asc" ? "Cambiar a orden descendente" : "Cambiar a orden ascendente"} onClick={() => setSortDirection((direction) => direction === "asc" ? "desc" : "asc")}>{sortDirection === "asc" ? "↑" : "↓"}</button><label><span>Agrupar</span><select value={groupBy} onChange={(event) => setGroupBy(event.target.value)}><option value="type">Tipo de bono</option><option value="percentage">Porcentaje</option><option value="none">Sin agrupación</option></select></label><button className="config-add" type="button" onClick={() => openEditor()}><Plus size={15} /> Nuevo bono</button></div></div>
